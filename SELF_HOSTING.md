...
Deploy Statica on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`statica` CLI** and runs the **agent daemon** on their own machine.
...
This clones the repository, starts all services via Docker Compose, installs the `statica` CLI, then configures it for localhost.
...
```bash
curl -fsSL https://raw.githubusercontent.com/statica-ai/statica/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to Statica Cloud

If you've been self-hosting and want to switch your CLI to [Statica Cloud](https://statica.ai):

```bash
statica setup
```

This reconfigures the CLI for statica.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
If you prefer configuring the CLI step by step instead of `statica setup`:
