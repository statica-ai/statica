...
3. **多 tab 互相覆盖**：`statica_workspace_id` 是全局 localStorage key，两个 tab 打开不同 workspace 会互相污染
...
2. **状态管理清理**：删除 `useWorkspaceStore.workspace` 作为独立状态，改为从 URL 派生；删除 `hydrateWorkspace` / `switchWorkspace` actions（切 workspace 变成纯导航）；删除 `localStorage["statica_workspace_id"]`
...
6. **Shareable URL 修复**：桌面端 `getShareableUrl` 当前生成 `https://www.statica.ai/issues/abc`（缺 slug），需要更新
7. **后端保留词校验**：slug 不能和前端顶级路由冲突（`login`、`onboarding`、`invite`、`api`、`settings` 等），后端创建时校验
8. **内部 markdown 链接兼容**：issue 评论里写的 `[foo](/issues/abc)` 触发的 `statica:navigate` 事件需要自动补当前 workspace slug
...
- 桌面端 `statica://auth/callback` — 认证回调，不涉及 workspace
