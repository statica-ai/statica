...
**Architecture:** `@statica/core` 自带完整初始化（API、stores、WS），不需要每个 app 调用 factory。`@statica/views` 包含所有页面和 layout。每个 app 只提供路由壳子。
...
方案：`@statica/core` 导出一个 `<CoreProvider>` 包裹整个应用。它内部自动完成所有初始化。配置通过环境变量（`VITE_API_URL` / `NEXT_PUBLIC_API_URL`）或 prop 注入。SSR-safe 的 localStorage wrapper 内置到 core 里作为默认 storage（`typeof window` 守卫对 Electron 无害）。
...
### Task 1: 在 `@statica/core` 里创建 CoreProvider
...
export function AuthInitializer({
  ...
  useEffect(() => {
    const token = defaultStorage.getItem("statica_token");
    if (!token) {
      onLogout?.();
    ...
    const api = getApi();
    api.setToken(token);
    const wsId = defaultStorage.getItem("statica_workspace_id");
    ...
    Promise.all([api.getMe(), api.listWorkspaces()])
      ...
      .catch((err) => {
        ...
        defaultStorage.removeItem("statica_token");
        defaultStorage.removeItem("statica_workspace_id");
        onLogout?.();
...
import { CoreProvider } from "@statica/core/platform";
...
- `from "@/platform/api"` → `from "@statica/core/api"` (use singleton proxy `api`)
- `from "@/platform/auth"` → `from "@statica/core/auth"` (use singleton `useAuthStore`)
- `from "@/platform/workspace"` → `from "@statica/core/workspace"` (use singleton `useWorkspaceStore`)
...
### Task 3: Extract `AppSidebar` to `@statica/views/layout`
...
- `import { useAuthStore } from "@statica/core/auth"` (singleton)
- `import { useWorkspaceStore } from "@statica/core/workspace"` (singleton)
- `import { api } from "@statica/core/api"` (singleton proxy)
- `import { useNavigation, AppLink } from "../navigation"` (relative within views)
- `import { useModalStore } from "@statica/core/modals"`
- All `@statica/ui` imports unchanged
...
```bash
git commit -m "refactor(views): extract shared AppSidebar to @statica/views/layout"
```

---

### Task 4: Extract `DashboardLayout` to `@statica/views/layout`
...
import { useEffect, type ReactNode } from "react";
import { useNavigationStore } from "@statica/core/navigation";
import { SidebarProvider, SidebarInset } from "@statica/ui/components/ui/sidebar";
import { WorkspaceIdProvider } from "@statica/core/hooks";
import { useAuthStore } from "@statica/core/auth";
import { useWorkspaceStore } from "@statica/core/workspace";
...
"use client";
import { DashboardLayout } from "@statica/views/layout";
import { StaticaIcon } from "@/components/statica-icon";
import { SearchCommand } from "@/features/search";

export default function Layout({ children }: { children: React.ReactNode }) {
  return (
    <DashboardLayout
      loadingIndicator={<StaticaIcon className="size-6" />}
      extra={<SearchCommand />}
    >
      {children}
    </DashboardLayout>
...
import { StaticaIcon } from "./statica-icon";
...
```bash
git commit -m "refactor(views): extract shared DashboardLayout to @statica/views/layout"
```
...
### Task 6: Extract `LoginPage` to `@statica/views/auth`
...
import { LoginPage } from "@statica/views/auth";
import { StaticaIcon } from "../components/statica-icon";
...
export function DesktopLoginPage() {
  ...
  return (
    <div className="flex h-screen flex-col">
      <TitleBar />
      <LoginPage
        logo={<StaticaIcon bordered size="lg" />}
...
```bash
git commit -m "feat(views): extract shared LoginPage to @statica/views/auth"
```
...
### Task 8: Create `@statica/views/agents`
...
**Key migration:** All `@/platform/*` imports → `@statica/core/*` singletons. All `@statica/ui` and `@statica/core` imports stay as-is. `@statica/views` imports become relative.
...
```bash
git commit -m "feat(views): extract agents page to @statica/views/agents"
```
...
export { AgentsPage as default } from "@statica/views/agents";
```

Commit: `refactor(web): replace agents page with @statica/views/agents import`
...
```typescript
// router.tsx
import { AgentsPage } from "@statica/views/agents";
{ path: "agents", element: <AgentsPage /> },
```

Commit: `feat(desktop): wire agents page from @statica/views`

---

## Phase 5: Extract Inbox Page (468 lines → shared module)

### Task 11: Create `@statica/views/inbox`
...
- `@/platform/*` → `@statica/core/*` singletons

Commit: `feat(views): extract inbox page to @statica/views/inbox`
...
export { InboxPage as default } from "@statica/views/inbox";
```

Commit: `refactor(web): replace inbox page with @statica/views/inbox import`
...
```typescript
import { InboxPage } from "@statica/views/inbox";
{ path: "inbox", element: <InboxPage /> },
```

Commit: `feat(desktop): wire inbox page from @statica/views`

---

## Phase 6: Extract Settings Page (1,277 lines → shared module)

### Task 14: Create `@statica/views/settings`
...
**Key migration:** Same pattern — `@/platform/*` → `@statica/core/*` singletons.

Commit: `feat(views): extract settings page to @statica/views/settings`
...
export { SettingsPage as default } from "@statica/views/settings";
```

Delete `apps/web/app/(dashboard)/settings/_components/` (all 6 files).

Commit: `refactor(web): replace settings page with @statica/views/settings import`
...
```typescript
import { SettingsPage } from "@statica/views/settings";
{ path: "settings", element: <SettingsPage /> },
```

Commit: `feat(desktop): wire settings page from @statica/views`
