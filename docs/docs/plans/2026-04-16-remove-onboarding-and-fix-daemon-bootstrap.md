...
```bash
git worktree add ../statica-onboarding-removal -b chore/remove-onboarding origin/main
cd ../statica-onboarding-removal
```
...
vi.mock("@statica/core/workspace/mutations", () => ({
...
```bash
pnpm --filter @statica/views exec vitest run workspace/create-workspace-form.test.tsx
```
...
import { Card, CardContent } from "@statica/ui/components/ui/card";
import { useCreateWorkspace } from "@statica/core/workspace/mutations";
import type { Workspace } from "@statica/core/types";
import {
...
import {
  ...
  DialogTitle,
  DialogDescription,
} from "@statica/ui/components/ui/dialog";
...
```bash
pnpm --filter @statica/views exec vitest run workspace/create-workspace-form.test.tsx
pnpm --filter @statica/views exec vitest run modals/create-workspace.test.tsx
```
...
```bash
pnpm --filter @statica/core exec vitest run paths/paths.test.ts paths/reserved-slugs.test.ts
```
...
import { paths } from "@statica/core/paths";
import { CreateWorkspaceForm } from "@statica/views/workspace/create-workspace-form";
...
```tsx
import { NoAccessPage } from "@statica/views/workspace/no-access-page";
...
**Files:**
- Modify: `server/cmd/statica/cmd_login.go` (line ~128)
...
```bash
cd server && go test ./cmd/statica/ -v
```
...
```bash
git add server/cmd/statica/cmd_login.go
...
import { workspaceListOptions } from "@statica/core/workspace/queries";
...
Without this, `pnpm typecheck` may still report the export as valid and subsequent imports from `@statica/views/onboarding` won't be caught as broken.
...
statica login --server-url http://localhost:8080
