...
**Goal:** Create `apps/desktop/` Electron app that reuses `@statica/core`, `@statica/ui`, `@statica/views` — identical UI to web, with a custom frameless title bar.
...
**Modify:** `package.json` (root) — add `"dev:desktop": "turbo dev --filter=@statica/desktop"` to scripts.
...
- Resolve monorepo packages (`@statica/core`, `@statica/ui`, `@statica/views`)
...
```json
{
  ...
  "dependencies": {
    "@statica/core": "workspace:*",
    "@statica/ui": "workspace:*",
    "@statica/views": "workspace:*",
...
```typescript
import type { StorageAdapter } from "@statica/core/types/storage";
...
```typescript
import { ApiClient } from "@statica/core/api/client";
import { setApiInstance } from "@statica/core/api";
import { createLogger } from "@statica/core/logger";
...
export const api = new ApiClient(API_BASE_URL, {
  ...
  onUnauthorized: () => {
    desktopStorage.removeItem("statica_token");
    desktopStorage.removeItem("statica_workspace_id");
...
const token = desktopStorage.getItem("statica_token");
if (token) api.setToken(token);
const wsId = desktopStorage.getItem("statica_workspace_id");
...
```typescript
import { createAuthStore, registerAuthStore } from "@statica/core/auth";
import { api } from "./api";
import { desktopStorage } from "./storage";
...
```typescript
import { createWorkspaceStore, registerWorkspaceStore } from "@statica/core/workspace";
...
```typescript
import { WSProvider } from "@statica/core/realtime";
...
import { ModalRegistry } from "@statica/views/modals/registry";
        ...
        <SidebarProvider className="flex-1">
          ...
          <SidebarInset className="overflow-hidden">
            {workspace ? (
              ...
              <div className="flex flex-1 items-center justify-center">
                <StaticaIcon className="size-6 animate-pulse" />
              </div>
            )}
          </SidebarInset>
        </SidebarProvider>
...
- `statica-icon.tsx` — Copy as-is (pure CSS, no dependencies)
- `app-sidebar.tsx` — Copy from `apps/web/app/(dashboard)/_components/app-sidebar.tsx`, change:
  ...
  - Any `next/link` → `AppLink` from `@statica/views/navigation` (check if already using AppLink)
...
@import "@statica/ui/styles/tokens.css";
...
// Extracted pages from @statica/views
import { IssuesPage } from "@statica/views/issues/components";
import { IssueDetail } from "@statica/views/issues/components";
import MyIssuesPage from "@statica/views/my-issues";
import RuntimesPage from "@statica/views/runtimes";
import SkillsPage from "@statica/views/skills";

// Placeholder pages (not yet extracted to @statica/views)
    ...
    <div className="flex flex-1 items-center justify-center">
      <div className="text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Coming soon — requires page extraction to @statica/views.
        </p>
      </div>
    </div>
  );
...
import { Toaster } from "@statica/ui/components/ui/sonner";
import { QueryProvider } from "@statica/core/provider";
...
import { Card, CardHeader, CardTitle, CardDescription, CardContent } from "@statica/ui/components/ui/card";
import { Input } from "@statica/ui/components/ui/input";
import { Button } from "@statica/ui/components/ui/button";
import { Label } from "@statica/ui/components/ui/label";
        ...
        <Card className="w-[380px]">
          <CardHeader className="text-center">
            <div className="mx-auto mb-4">
              <StaticaIcon bordered size="lg" />
            </div>
            <CardTitle>Sign in to Statica</CardTitle>
            <CardDescription>
              {step === "email"
...
```json
{
  "extends": "@statica/tsconfig/base.json",
  "compilerOptions": {
    "outDir": "out",
...
```json
{
  "extends": "@statica/tsconfig/react-library.json",
  "compilerOptions": {
    "outDir": "out",
    "baseUrl": ".",
    "paths": {
...
1. **Extract fat pages to @statica/views:**
...
2. **App sidebar extraction:** Copy from web or extract shared sidebar to `@statica/views/layout/`
