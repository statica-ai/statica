...
- Workspace identity lives in 4 synchronized copies: `localStorage["statica_workspace_id"]`, `useWorkspaceStore.workspace`, `api._workspaceId`, `_currentWsId` module var in `packages/core/platform/workspace-storage.ts`. Any hand-synchronization drift = bug.
...
- `localStorage["statica_workspace_id"]` is a single global key. Two tabs on different workspaces overwrite each other on refresh.
...
| Deleted references to `statica_workspace_id` | ~10 | After URL becomes source of truth |
...
User action → switchWorkspace(ws) → mutates:
  ...
  3. localStorage["statica_workspace_id"]
...
**Invariants enforced:**
- No file reads `localStorage["statica_workspace_id"]` after this PR — it stops being written too.
...
```bash
pnpm --filter @statica/core exec vitest run paths/paths.test.ts
```
...
Modify `packages/core/package.json` exports (if there's an explicit exports map; otherwise this is consumed via `@statica/core/paths`).
...
```bash
pnpm --filter @statica/core exec vitest run paths/hooks.test.tsx
```
...
```bash
pnpm --filter @statica/core exec vitest run paths/
```
...
- Delete: `const wsId = storage.getItem("statica_workspace_id");`
- Delete: `useWorkspaceStore.getState().hydrateWorkspace(wsList, wsId);` (both branches)
- Delete: `storage.removeItem("statica_workspace_id");` (in catch block)
...
export function AuthInitializer({
  ...
  useEffect(() => {
    ...
    const token = storage.getItem("statica_token");
    if (!token) {
      onLogout?.();
    ...
    Promise.all([api.getMe(), api.listWorkspaces()])
      ...
      .catch((err) => {
        ...
        api.setToken(null);
        storage.removeItem("statica_token");
        onLogout?.();
        useAuthStore.setState({ user: null, isLoading: false });
      });
  }, []);
...
import { api } from "@statica/core/api";
import {
  WorkspaceSlugProvider,
  workspaceBySlugOptions,
} from "@statica/core/paths";
import { useWorkspaceStore } from "@statica/core/workspace";
import { setCurrentWorkspaceId, rehydrateAllWorkspaceStores } from "@statica/core/platform/workspace-storage";
...
```bash
pnpm --filter @statica/web build 2>&1 | head -40
```
...
export function middleware(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("statica_session");
...
```bash
grep -rn "statica_session\|setLoggedInCookie\|cookie" apps/web/features/auth/
```

Find the actual cookie name. Update `middleware.ts` if it's different (likely `statica_logged_in` or similar — the cookie only needs to indicate presence, not carry auth).
...
import { useAuthStore } from "@statica/core/auth";
import { useQuery } from "@tanstack/react-query";
import { workspaceListOptions } from "@statica/core/workspace";
import { paths } from "@statica/core/paths";
...
- Line 67: `localStorage.getItem("statica_workspace_id")` → DELETE
...
`(auth)/login/page.tsx`:
- Line 32: `localStorage.getItem("statica_workspace_id")` → DELETE
...
When a markdown link like `[foo](/issues/abc)` is clicked inside an issue's content, it dispatches `statica:navigate` with path `/issues/abc`. After the refactor, this is not a valid route — it needs workspace slug.
...
```ts
import { paths, isGlobalPath } from "@statica/core/paths";

/** Open a link — internal paths dispatch statica:navigate, external open new tab. */
export function openLink(href: string, currentSlug?: string | null): void {
  if (href.startsWith("/")) {
    ...
    window.dispatchEvent(
      new CustomEvent("statica:navigate", { detail: { path } }),
    );
  } else {
...
```ts
// Add "statica_navigation" to WORKSPACE_SCOPED_KEYS array
```
...
Current (line 67, 110): `getShareableUrl: (path: string) => \`https://www.statica.ai${path}\``
...
However, double-check by testing: open an issue, click "copy link", paste it. Verify it's `https://www.statica.ai/{slug}/issues/{id}`.

If the web URL domain is configurable (e.g. different staging domains), extract the domain to an env var:

```tsx
const WEB_URL = import.meta.env.VITE_WEB_URL || "https://www.statica.ai";
// ...
...
const lastWsId = localStorage.getItem("statica_workspace_id");
...
### Task 23: Remove all `statica_workspace_id` references

Run:

```bash
grep -rn "statica_workspace_id" packages/ apps/ | grep -v ".test." | grep -v "docs/plans/"
```
...
```bash
git add -A
git commit -m "chore: remove all statica_workspace_id references"
```
...
- [ ] Desktop: copy-link generates `https://www.statica.ai/{slug}/issues/{id}`
