...
All other packages will `"extends": "@statica/tsconfig/react-library.json"`.
...
**Files:**
- Create: `packages/core/package.json` (name: @statica/core, deps: react, zustand, @tanstack/react-query, sonner)
- Create: `packages/core/tsconfig.json` (extends @statica/tsconfig/react-library.json)
...
| Pattern | Replacement |
|---|---|
| `@/shared/types` | `@statica/core/types` |
| `@/shared/api"` (singleton usage) | `@/platform/api"` |
| `@/shared/logger` | `@statica/core/logger` |
| `@/shared/utils` | `@statica/core/utils` |
| `@/shared/hooks/` | `@statica/core/hooks/` |
| `@core/` | `@statica/core/` |
| `@/features/auth"` (useAuthStore) | `@/platform/auth"` |
| `@/features/workspace"` (useWorkspaceStore) | `@/platform/workspace"` |
| `@/features/workspace"` (useActorName) | `@statica/core/workspace/hooks"` |
| `@/features/realtime` | `@statica/core/realtime` |
| `@/features/navigation` | `@statica/core/navigation` |
| `@/features/modals"` (store) | `@statica/core/modals"` |
| `@/features/issues/store` | `@statica/core/issues` |
| `@/features/issues/stores/` | `@statica/core/issues/stores/` |
| `@/features/issues/config` | `@statica/core/issues/config` |

**Also:**
- Add `"@statica/core": "workspace:*"` to `apps/web/package.json`
- Add `transpilePackages: ["@statica/core"]` to `next.config.ts`
...
**Files:**
- Create: `packages/ui/package.json` (name: @statica/ui, deps: all @radix-ui/*, clsx, tailwind-merge, lucide-react, emoji-mart, react-markdown, shiki, etc.)
...
- Update `globals.css`: replace inline tokens with `@import "@statica/ui/styles/tokens.css"` + add `@source` directives for packages
...
| Pattern | Replacement |
|---|---|
| `@/components/ui/` | `@statica/ui/components/ui/` |
| `@/components/common/` | `@statica/ui/components/common/` |
| `@/components/markdown` | `@statica/ui/markdown` |
| `@/lib/utils` | `@statica/ui/lib/utils` |
| `@/hooks/use-mobile` | `@statica/ui/hooks/use-mobile` |
| `@/hooks/use-auto-scroll` | `@statica/ui/hooks/use-auto-scroll` |
| `@/hooks/use-scroll-fade` | `@statica/ui/hooks/use-scroll-fade` |

**Also:**
- Add `"@statica/ui": "workspace:*"` to `apps/web/package.json`
- Add `"@statica/ui"` to `transpilePackages` in `next.config.ts`
- Update `apps/web/components.json` aliases to point to `@statica/ui`

**Delete:** `components/ui/`, `components/common/`, `components/markdown/`, `hooks/`, `lib/utils.ts`

**Keep:** `components/{theme-provider,theme-toggle,statica-icon,loading-indicator,spinner,locale-sync}.tsx`
...
- `packages/views/package.json` (deps: @statica/core, @statica/ui, @dnd-kit/*, @tiptap/*, sonner, recharts)
...
| Pattern | Replacement |
|---|---|
| `@/features/issues/components` | `@statica/views/issues/components` |
| `@/features/issues/hooks/` | `@statica/views/issues/hooks/` |
| `@/features/editor` | `@statica/views/editor` |
| `@/features/modals/` (components) | `@statica/views/modals/` |
| `@/features/my-issues` | `@statica/views/my-issues` |
| `@/features/skills` | `@statica/views/skills` |
| `@/features/runtimes` | `@statica/views/runtimes` |

**Also:**
- Add `"@statica/views": "workspace:*"` to `apps/web/package.json`
- Add `"@statica/views"` to `transpilePackages`
...
# Package constraints
grep -r "@statica/core" packages/ui/ || echo "PASS: ui/ has zero core imports"
...
│   ├── core/              # @statica/core — 三端共用 (零 react-dom)
...
│   ├── ui/                # @statica/ui — Web+Desktop 共用 (零业务逻辑)
...
│   │   ├── lib/utils.ts   #   cn()
│   │   └── styles/tokens.css
│   └── views/             # @statica/views — Web+Desktop 共用页面
...
│       └── components/    #   theme-provider, statica-icon 等 app 级组件
├── turbo.json
└── pnpm-workspace.yaml
```
