...
This document covers advanced configuration for self-hosted Statica deployments. For the quick start guide, see [SELF_HOSTING.md](SELF_HOSTING.md).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string | `postgres://statica:statica@localhost:5432/statica?sslmode=disable` |
| `JWT_SECRET` | **Must change from default.** Secret key for signing JWT tokens. Use a long random string. | `openssl rand -hex 32` |
| `FRONTEND_ORIGIN` | URL where the frontend is served (used for CORS) | `https://app.example.com` |

### Email (Required for Authentication)

Statica uses email-based magic link authentication via [Resend](https://resend.com).

| Variable | Description |
|----------|-------------|
| `RESEND_API_KEY` | Your Resend API key |
| `RESEND_FROM_EMAIL` | Sender email address (default: `noreply@statica.ai`) |
...
Statica requires PostgreSQL 17 with the pgvector extension.
...
By default, Statica works on `localhost`. If you access it from another machine on the LAN (e.g. `http://192.168.1.100:3000`), you need to tell the backend to accept that origin:
