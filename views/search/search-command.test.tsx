...
const {
  ...
  mockGetShareableUrl: vi.fn((p: string) => `https://app.statica/${p}`),
  mockWorkspaces: {
...
vi.mock("@statica/core", () => ({
  useWorkspaceId: () => "ws-test",
}));

vi.mock("@statica/core/paths", () => ({
  paths: {
...
vi.mock("@statica/core/issues/queries", () => ({
...
vi.mock("@statica/ui/components/common/theme-provider", () => ({
...
describe("SearchCommand", () => {
  beforeEach(() => {
    ...
    mockGetShareableUrl.mockReset().mockImplementation((p: string) => `https://app.statica/${p}`);
  ...
  it("copies issue link and identifier when on an issue detail route", async () => {
    ...
    expect(mockClipboardWrite).toHaveBeenCalledWith("https://app.statica//ws-test/issues/issue-1");
