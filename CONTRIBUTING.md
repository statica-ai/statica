...
The daemon authenticates using the CLI's stored token (`statica login`).
...
Example: worktree at `../statica-feat-auth` produces profile
...
1. Compiles the `statica` CLI from `server/cmd/statica` into
   `apps/desktop/resources/bin/statica`
...
Nothing in this flow touches the system-installed `statica` or the default
`~/.statica/config.json`:
