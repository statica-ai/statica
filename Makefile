.PHONY: dev server daemon cli statica build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down selfhost selfhost-stop
...
POSTGRES_DB ?= statica
POSTGRES_USER ?= statica
POSTGRES_PASSWORD ?= statica
POSTGRES_PORT ?= 5432
PORT ?= 8080
FRONTEND_PORT ?= 3000
...
selfhost:
	...
	@echo "==> Starting Statica via Docker Compose..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		...
		echo "  brew install statica-ai/tap/statica"; \
		echo "  statica setup self-host"; \
	else \
		echo ""; \
...
selfhost-stop:
	@echo "==> Stopping Statica services..."
...
build:
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/statica ./cmd/statica
