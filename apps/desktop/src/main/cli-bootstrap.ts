...
// Desktop bootstraps its own copy of the `statica` CLI into userData on first
...
const GITHUB_LATEST_BASE =
  "https://github.com/statica-ai/statica/releases/latest/download";
...
function platformAssetName(): string {
  ...
  return `statica_${os}_${arch}.${ext}`;
}

function binaryName(): string {
  return process.platform === "win32" ? "statica.exe" : "statica";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `statica-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `statica` binary. If one is already present at
