...
const PREFS_PATH = join(homedir(), ".statica", "desktop_prefs.json");
...
// Keep the Go impl in sync: server/cmd/statica/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".statica", "profiles", profile)
    : join(homedir(), ".statica");
}
...
// Sidecar file that records which Statica user the cached PAT in config.json
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["statica.exe"] : ["statica"];
 ...
 *   to `apps/desktop/resources/bin/statica`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Statica.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "statica.exe" : "statica";
...
/**
 * Returns a usable `statica` binary path. Priority:
 ...
 *   5. `statica` on PATH (dev convenience / user-installed via brew).
 ...
 * `statica version --output json`. Cached for the process lifetime — the
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Statica Desktop" }),
  });
  if (!res.ok) {
...
async function savePrefs(prefs: DaemonPrefs): Promise<void> {
  const dir = join(homedir(), ".statica");
...
async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "statica CLI is not installed" };
