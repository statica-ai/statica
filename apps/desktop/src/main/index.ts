...
// statica CLI can find agent binaries like claude/codex/opencode. Must run
...
function handleDeepLink(url: string): void {
  try {
    ...
    // statica://auth/callback?token=<jwt>
...
// single-worktree case still lands at "Statica Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Statica Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Statica Canary";
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    electronApp.setAppUserModelId(
      is.dev ? "ai.statica.desktop.dev" : "ai.statica.desktop",
    );
