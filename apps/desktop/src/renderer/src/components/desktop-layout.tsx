...
import { AppSidebar } from "@statica/views/layout";
import { SearchCommand, SearchTrigger } from "@statica/views/search";
import { ChatFab, ChatWindow } from "@statica/views/chat";
import { WorkspaceSlugProvider } from "@statica/core/paths";
import { getCurrentSlug, subscribeToCurrentSlug } from "@statica/core/platform";
...
function useInternalLinkHandler() {
  useEffect(() => {
    ...
    window.addEventListener("statica:navigate", handler);
    return () => window.removeEventListener("statica:navigate", handler);
  }, []);
}
