...
import { workspaceKeys, workspaceListOptions } from "@statica/core/workspace/queries";
import { api } from "@statica/core/api";
import { ThemeProvider } from "@statica/ui/components/common/theme-provider";
import { StaticaIcon } from "@statica/ui/components/common/statica-icon";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.statica config).
  useEffect(() => {
    window.daemonAPI.setTargetApiUrl(DAEMON_TARGET_API_URL);
  }, []);

  // Listen for invite IDs delivered via deep link (statica://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (statica://auth/callback?token=...).
  ...
  useEffect(() => {
    if (!user) return;
    const token = localStorage.getItem("statica_token");
    if (!token) return;
