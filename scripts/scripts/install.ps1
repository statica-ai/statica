# Statica installer for Windows — one command to get started.
#
# Install CLI (default): connects to statica.ai
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/statica-ai/statica/releases/latest" -ErrorAction Stop
...
function Install-CliBinary {
    Write-Info "Installing Statica CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Statica requires a 64-bit Windows installation."
    }
    $arch = "amd64"
    ...
    $url = "https://github.com/statica-ai/statica/releases/download/$latest/statica_windows_$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "statica-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "statica.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/statica-ai/statica/releases/download/$latest/checksums.txt"
    try {
        $checksums = Invoke-WebRequest -Uri $checksumUrl -UseBasicParsing -ErrorAction Stop
        $zipFile = Join-Path $tmpDir "statica.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $expectedLine = ($checksums.Content -split "`n") | Where-Object { $_ -match "statica_windows_$arch\.zip" } | Select-Object -First 1
        if ($expectedLine) {
    ...
    Expand-Archive -Path (Join-Path $tmpDir "statica.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".statica\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "statica.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "statica.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "statica.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "statica.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Statica CLI installed to $binDir\statica.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "statica") {
        $currentVer = (statica version 2>$null) -replace '.*?(v[\d.]+).*','$1'
        ...
        if ($isUpToDate) {
            Write-Ok "Statica CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Statica CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = (statica version 2>$null) -replace '.*?(v[\d.]+).*','$1'
        Write-Ok "Statica CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "statica")) {
        Write-Fail "CLI installed but 'statica' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Statica self-hosting requires Docker and Docker Compose.
...
function Install-Server {
    Write-Info "Setting up Statica server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Statica repository..."
    ...
    Write-Info "Starting Statica services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Statica server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Statica - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Statica CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     statica setup               " -NoNewline; Write-Host "# Connect to Statica Cloud (statica.ai)" -ForegroundColor DarkGray
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Statica - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Statica server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     statica setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Statica services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Statica installation found at $InstallDir"
    }

    if (Test-CommandExists "statica") {
        try {
            statica daemon stop 2>$null
