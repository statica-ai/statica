...
**关联 issue**：[#951](https://github.com/statica-ai/statica/issues/951)
...
另外 `packages/core/platform/storage-cleanup.ts:10-19` 的 `WORKSPACE_SCOPED_KEYS` 列表里也漏了 `statica_navigation`。
...
3. `packages/core/platform/storage-cleanup.ts:10-19` 的 `WORKSPACE_SCOPED_KEYS` 数组里加 `"statica_navigation"`
